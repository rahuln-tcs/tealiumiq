...
class Tealiumiq {
  ...
  public function getProfile() {
    ...
    if ($node instanceof Node) {
      $content_type_profile = $this->config->get('content_type_profile');
      if ($content_type_profile) {
        $profiles = explode("\n", $content_type_profile);
        $profile_array = [];
        foreach ($profiles as $value) {
          $value = explode("|", $value);
          $profile_array[$value[0]] = $value[1];
        }
        if (!empty($profile_array) && array_key_exists($node->bundle(), $profile_array)) {
          $profile = trim($profile_array[$node->bundle()]);
        }
      }
    }
    return $profile;
